...
from utils.db_utils import delete_pharmacy
import traceback
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if 'pharmacy_id' in req_body and req_body['pharmacy_id'] and type(req_body['pharmacy_id']) == int:
            is_success, detail = delete_pharmacy(cursor, req_body['pharmacy_id'])
        else:
            return func.HttpResponse(body =json.dumps({"error": "No valid 'pharmacy_id' provided."}), status_code=400)
